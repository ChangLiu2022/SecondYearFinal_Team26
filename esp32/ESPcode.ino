...
void sendPostRequest(uint16_t vPostData[78]) { // NCOORDS
  ...
  int j=0;
  printLocalTime();
  // Append the data for each beacon type to the payload
  for (int i = 0; i < 78; i += 2) {
    if (i != 0) {
      payload += ",";
    }
    //printLocalTime();
    payload += "{\"timestamp\":\"" + String(currentTime) + "\", \"BeaconType\":\"" + getBeaconType(j) + "\", \"xcordinate\":" + String(vPostData[i]) + ", \"ycordinate\":" + String(vPostData[i + 1]) + "}";
    j++;
  }
...
void loop() {
  ...
  if (FPGA.available()) {
    size = 0;
    int state = 0;
    //coordsData[0] == 0x00 && coordsData[1] == 0x45 && coordsData[2] == 0x4e && coordsData[3] == 0x44
    //DETECT END SEQUENCE
    if (start == 1){
      //Serial.print("AWAITING END");
          while(state != 4){
            FPGA.readBytes(&aaaa, 1);
           ...
           // Serial.print("   State: ");
            ...
            if(state == 0 && aaaa == 0x44){
              state = 1;
            }
            else if(state == 1 && aaaa == 0x4e){
              state = 2;
            }
            else if(state == 2 && aaaa == 0x45){
              state = 3;
            }
